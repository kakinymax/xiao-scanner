...
## 2026-05-22 - [XSS via unescaped innerHTML in Error Handling and AI Responses]
**Vulnerability:** The application used `innerHTML` to directly render unescaped text received from the Gemini API and JavaScript Error messages.
**Learning:** Even internal errors or trusted external API text can contain unsanitized characters that execute maliciously if rendered via `innerHTML`. Always sanitize external input.
**Prevention:** Implement and use a utility function like `escapeHTML` to convert dangerous characters (like `<`, `>`, `&`, `"`, `'`) to their HTML entity equivalents before rendering to the DOM when using `innerHTML`.
